...
const start = async () => {
  try {
    ...
    const getBackupFile = readdirSync(join(__dirname, "../backup/")).filter(
      (name) => name.endsWith("")
    );
    ...
    const getSelectedValue = val[0].name.substring(
      val[0].name.indexOf("DB"),
      val[0].name.length
    );
    ...
    const fileNames = sortArray(
      readdirSync(targetDirname).filter((name) => name.endsWith(".json"))
    );

    const models = fileNames.filter((item) =>
      mongoose.modelNames().includes(item.replace(".json", ""))
    );
    ...
    for (const model of models) {
      ...
      for (const document of data) {
        ...
        const restore = await mongoose
          .model(modelName)
          .findOneAndUpdate(query, update, options)
          .catch((error) => console.error(error));
      }
      console.log(
        `Successfully restoring: ${data.length} data on collection ${modelName}`
      );
    }
